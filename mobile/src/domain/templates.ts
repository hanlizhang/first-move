...
type TemplateSeed = Pick<FirstMoveTemplate, "text" | "durationMinutes">;
...
export const FIRST_MOVE_TEMPLATES: FirstMoveTemplate[] = STUCK_STATES.flatMap((stuckState) =>
  DIRECTIONS.flatMap((direction) => [
    {
      id: `${slug(stuckState)}-${slug(direction)}-1`,
      stuckState,
      direction,
      ...specificMoves[stuckState][direction],
    },
    {
      id: `${slug(stuckState)}-${slug(direction)}-2`,
      stuckState,
      direction,
      ...alternateMoves[direction],
    },
    ...(waterResetStates.includes(stuckState)
      ? [
          {
            id: `${slug(stuckState)}-${slug(direction)}-water`,
            stuckState,
            direction,
            ...waterReset,
          },
...
export function templatesFor(
  ...
  return FIRST_MOVE_TEMPLATES.filter(
    (template) =>
      template.stuckState === stuckState && template.direction === direction,
  );
}
...
export function easierTemplateFor(
  ...
  const selected = options[(currentIndex + 1) % options.length] ?? options[0];
  if (!selected) {
