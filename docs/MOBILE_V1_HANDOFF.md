...
The Focus parent selector is now one compact field that opens a searchable modal with separate Tasks and Habits, No linked item, local/canonical source labels, and a clear selected state. It still writes only `linkedTaskId` or `linkedHabitId`.
...
M1E authenticated cloud writes must add the explicit Phase B2 Start fresh / Import this device / Use cloud progress choices; an explicit activation decision for M1D account-local rows already coexisting with an initialized canonical workspace; a durable device identity and retry-safe ordered mutation/outbox path; validated canonical hydration after acknowledgements; parent-first Task/Habit create/update/tombstone delivery, including materializing a locally deleted but historically referenced parent before its tombstone; weekday and local-date completion convergence; active-pending-only Intent serialization; and server-authoritative task/habit reward creation. It must never submit an unreconciled account-local array to the full-snapshot RPC because that would tombstone absent canonical rows. After explicit activation it must make canonical parents editable under the same UUID, without copying or automatically merging Guest data, preserve offline work on failure, and pass Web/iOS/Android same-account and owner-isolation acceptance.
