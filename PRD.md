...
- **Active Days:** keep the existing Active Day system unchanged as the sole Cat growth and milestone day counter. Future heart mechanics do not replace, duplicate, or redefine Active Days.
- **Today's Hearts:** every local date starts at `♡♡♡`. A qualifying direct Cat interaction earns `+1` heart, including feeding, playing, tricks, and other meaningful direct interactions, up to three earned hearts per local date. After the third heart, every Cat interaction remains fully available but earns no additional heart that day. Hearts are never lost, missed days have no penalty, and no streak is required.
- **Total Hearts / Bond:** every earned daily heart contributes to one cumulative lifetime total representing companionship or bond, displayed conceptually like `Together · 128 ♥`. Total Hearts never decrease. There is no separate Care Day counter and no numbered Bond Level system.
- **Little Finds:** quirky collectible gifts such as a leaf, feather, pom-pom, tiny plush mouse, pebble, ribbon, acorn, shell, mysterious sock, or paperclip remain deferred. They have no point value and cannot be sold. Later eligibility may use Active Day milestones, cumulative Total Hearts, or bounded daily-heart activity. Authenticated selection must later be deterministic, idempotent, and server-authoritative rather than retryable client randomness; Finds never punish absence or require daily login.
- **Celebrations:** deferred celebration UI may distinguish a compact Coin gain, a larger `Active Day +1` moment, a daily `Heart +1` moment, a full Cat milestone unlock, and a Little Find reveal. Simultaneous rewards use one ordered or combined celebration queue rather than stacked blocking full-screen modals.
- Today's Hearts, Total Hearts / Bond, Little Finds, and celebration persistence, selection, schema, and UI remain designed only. Cat v1B does not implement any Hearts, Little Finds, or celebration code or database schema.
